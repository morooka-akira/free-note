...
impl IOSelector {
    fn new() -> Arc<Self> {
        let s = IOSelector {
            ...
            event: unsafe { eventfd(0, EfdFlags::empty()).unwrap() },
        };
    ...
    fn add_event(
        ...
        // EPOLLONESHOTを指定して、、一度イベントが発声すると
        // そのdfへのイベントは再設定するまで通知されないようになる
    ...
    fn rm_event(&self, fd: RawFd, wakers: &mut HashMap<RawFd, Waker>) {
        let epooll_del = EpollOp::EpollCtlDel;
        let mut ev = EpollEvent::new(EpollFlags::empty(), fd as u64);
        epoll_ctl(self.epfd, epooll_del, fd, &mut ev).ok();
        wakers.remove(&fd);
    }
